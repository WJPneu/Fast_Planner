...
private:
  ...
  /*
    sync是synchronize的缩写意思为同步
    sync_policies 同步策略
    Synchronizer 同步器
    message_filters::sync_policies::ApproximateTime 是 message_filters 库提供的一个预定义的同步策略类，用于近似时间同步多个消息类型。
  */
  ...
  /*
    下面两段代码创建了个同步策略将图像和位姿信息进行同步，并创建了同步器SynchronizerImagePose
  */
  ...
  /*
    ros::Timer 是 ROS 提供的定时器类，它允许在指定的时间间隔内周期性地执行回调函数。
    这些定时器对象可以用于在ROS节点中实现定时执行的任务
  */
